...
<body>
    <div class="index-header">
        <ul class="bar">
            ...
            <li>
                <a href="Notification.php"> Notification(<?php if (isset($total_noti)) {
                                                                echo "$total_noti";
                                                            } ?>)</a>
            </li>
            <li>
    ...
    <div class="right">
        <div class="mind">
            ...
            $select_post = "SELECT * FROM user_post ORDER BY 1 DESC";
            $run_select_post = mysqli_query($conn, $select_post);
            $total = 0;
            if ($run_select_post) {
            ...
            if (isset($_REQUEST["page"])) {
                ...
                for ($x = 0; $x <= $sow; $x++) {
                    ...
                    $select_post = "SELECT * FROM user_post ORDER BY 1 DESC limit $y,4;";
                    $run_select_post = mysqli_query($conn, $select_post);
                    $total = 0;
                    if ($run_select_post) {
                        $total = mysqli_num_rows($run_select_post);
                ...
                $select_post = "SELECT * FROM user_post ORDER BY 1 DESC limit 0,4;";
                $run_select_post = mysqli_query($conn, $select_post);
                $total = 0;
                if ($run_select_post) {
                    $total = mysqli_num_rows($run_select_post);
            ...
            ?>
                <div class="all-post">
                    ...
                    if ($cook == $puser) { ?>
                        ...
                        <a href='edit_post.php<?php echo "?post_id=$pid" ?>' class="comment">Edit</a>
                        <a href='comment.php<?php echo "?post_id=$pid" ?>' class="comment">Comment</a>
                        <a href='delete_post.php<?php echo "?post_id=$pid" ?>' class="comment">Delete</a>
                    <?php } else { ?>
                        <?php
            ...
            <?php  }
            for ($i = 1; $i <= $sow; $i++) {
                echo "<a href='home.php?page=$i' class='pages'>$i</a> ";
            } ?>
        </div>
    </div>
