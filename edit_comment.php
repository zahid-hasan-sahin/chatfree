...
<body>
    ...
    <div class="right">
        ...
        $select = "SELECT * FROM user_comment WHERE comment_id=$comment_id";
        $run_select_post = mysqli_query($conn, $select);
        while ($mydata = mysqli_fetch_array($run_select_post)) {
            $cfname = $mydata["fname"];
            $clname = $mydata["lname"];
            $ccomment = $mydata["comment"];
            $ctime = $mydata["time"];
            $cuser = $mydata["user_id"];
            $cid = $mydata["post_id"];
            $select = "SELECT * FROM `user_info` WHERE user_id='$cuser'";
            $run_select = mysqli_query($conn, $select);
            while ($mydata = mysqli_fetch_array($run_select)) {
                $ppic = $mydata["profic"];
            }
        ...
        ?>
            <div class="all-post">
                <h2 class="posted-by">comented By You On<?php echo " $ctime"; ?></h2>
                <img src='<?php echo "img/$ppic.jpg" ?>' style="width=60px;height:60px;">
                <h2 class="post_caption">Post:</h2>
                <h3 class="full-post"><?php echo "$ccomment"; ?></h3>
                <form action="edit_comment_core.php" method="get">
                    <input type="text" name="ucomment" value='<?php echo " $ccomment" ?>' class="comment_text">
                    <input type="hidden" name="comment_id" value='<?php echo "$comment_id" ?>'>
                    <input type="hidden" name="post_id" value='<?php echo "$cid" ?>'>
