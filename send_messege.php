...
<body>
    ...
    <div class="msg_right">
        <div class="view_msg">
            <?php
            $select = "SELECT * FROM `user_info` WHERE user_id='$to_id'";
            $run_select = mysqli_query($conn, $select);
            while ($mydata = mysqli_fetch_array($run_select)) {
                $tofname = $mydata["fname"];
                $tolname = $mydata["lname"];
            }
            ?>
            <h2>Send A Messege To <a href='search.php?id=<?php echo "$to_id"; ?>'><?php echo "$tofname $tolname" ?></a> </h2>
            <form method="post" action="messege_core.php">
                <input type="text" class="type_sub" name="subject" placeholder="Type A Subject"><br><br>
                <input type="text" class="type_msg" name="messege" placeholder="Type A Messege"><br>
                <input type="hidden" name="id" value='<?php echo "$to_id"; ?>'>
                <input type="submit">
            </form>
        </div>
