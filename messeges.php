...
<body>
    ...
    <div class="right">
        <?php
        $updata = "UPDATE `user_messege` SET `status`='seen' WHERE to_id='$cook'";
        $run_update = mysqli_query($conn, $updata);
        $select_total = "SELECT * FROM user_post where user_id='$cook';";
        $run_select_total = mysqli_query($conn, $select_total);
        $total_post = 0;
        if ($run_select_total) {
        ...
        ?>
        <?php
        $view_info = "SELECT * FROM `user_info`";
        $run_view_info = mysqli_query($conn, $view_info);
        while ($alldata = mysqli_fetch_array($run_view_info)) {
            $allsdata = $alldata["user_id"];

            $view_msg = "SELECT * FROM `user_messege` WHERE (to_id='$allsdata' and from_id='$dbid' ) or (to_id='$dbid' and from_id='$allsdata') order by 1 desc limit 0,1";
            ...
            while ($mydata = mysqli_fetch_array($run_view_msg)) {
                ...
                $view_profile = "SELECT * FROM `user_info` WHERE id='$to_user';";
        ...
        ?>
                <div class="all-messege">
                    <h2> <?php if ($from_user == $cook) {
                                echo "You Send A Messege:"; ?><a href='search.php?<?php echo "id=$tid"; ?>'><?php echo "$ffname $flname";
                                                                                                        } else {
                                                                                                            echo "A Messege From :"; ?><a href='search.php?<?php echo "id=$fid"; ?>'><?php echo "$tfname $tlname";
                                                                                                                                                                                                            } ?>
                                </a></h2><br>
                    <h3>Subject:<?php echo "$from_sub"; ?></h3> <br>
                    <a href='view_messege.php?id=<?php if ($from_user == $cook) {
                                                        echo "$to_user";
                                                    } else {
                                                        echo "$fid";
                                                    } ?>'> <input class="view" type="submit" value="View Full Messege"></a>
                </div><?php }
                } ?>
    </div>
